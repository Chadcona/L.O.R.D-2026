...
def draw_menu_box(surface, rect, alpha=230):
    ...
    # Outer border (bright)
    pygame.draw.rect(box, (*MENU_BORDER_OUTER, alpha), (0, 0, w, h), 2)
    # Inner border (darker)
    pygame.draw.rect(box, (*MENU_BORDER_INNER, alpha), (2, 2, w - 4, h - 4), 1)

    # Corner highlights
    for cx, cy in [(1, 1), (w - 2, 1), (1, h - 2), (w - 2, h - 2)]:
        box.set_at((cx, cy), (*WHITE, alpha))

    surface.blit(box, (x, y))


def draw_text(surface, text, x, y, color=WHITE, size=16, center=False, shadow=True):
...
class MenuCursor:
    ...
    def update(self, dt):
        ...
        self.bob_offset = self.frame * 2

    def draw(self, surface, x, y, color=WHITE):
        """Draw a small arrow cursor."""
        points = [
            (x + self.bob_offset, y),
            (x + 6 + self.bob_offset, y + 4),
            (x + self.bob_offset, y + 8),
        ]
...
class SelectionMenu:
    ...
    def __init__(self, items, x, y, spacing=16, font_size=16, color=WHITE,
                 highlight_color=GOLD, show_box=True, box_padding=8):
        self.items = items  # List of {"label": str, "value": any, "enabled": bool}
        self.x = x
        self.y = y
    ...
    def draw(self, surface):
        if self.show_box:
            ...
            box_w = max_width + self.box_padding * 2 + 16  # extra for cursor
        ...
        for i, item in enumerate(self.items):
            ...
            if i == self.selected and enabled:
                self.cursor.draw(surface, self.x, item_y + 1, self.highlight_color)
                draw_text(surface, item["label"], self.x + 12, item_y,
...
class DialogueBox:
    ...
    def __init__(self, x=8, y=170, width=304, height=62):
    ...
    def draw(self, surface):
        ...
        if self.speaker:
            draw_text(surface, self.speaker, text_x, text_y, GOLD, 14)
            text_y += 14
        ...
        for word in words:
            ...
            if font.size(test)[0] > max_width:
                draw_text(surface, line.strip(), text_x, text_y, WHITE, 14, shadow=True)
                text_y += 14
                line = word + " "
            else:
                line = test
        ...
        if self.finished:
            ...
            if indicator_time % 2 == 0:
                draw_text(surface, "v", x + w - 14, y + h - 14, WHITE, 12)
